...
import it.unibz.udig.omtg.gef.parts.OMTPDEditPartFactory;
import it.unibz.udig.omtg.pd.model.OMTPresentationDiagram;
import it.unibz.udig.omtg.pd.model.PDPackage;
import it.unibz.udig.omtg.pd.model.provider.PDItemProviderAdapterFactory;
...
public class OMTPresentationDiagramEditor extends GraphicalEditorWithFlyoutPalette
{

	private Resource				theResource;
	private OMTPresentationDiagram	theDiagram;

	public OMTPresentationDiagramEditor()
	{
	...
	{
		return new OMTPresentationDiagramGraphicalEditorPalette();
	}
	...
	protected void configureGraphicalViewer( )
	{
		...
		getGraphicalViewer().setEditPartFactory( new OMTPDEditPartFactory() );
	}
			...
			if ( propertyPage == null )
			{
				...
				IPropertySourceProvider sourceProvider = new IPropertySourceProvider()
				{
					...
					IPropertySourceProvider	modelPropertySourceProvider	= new AdapterFactoryContentProvider(
																				new PDItemProviderAdapterFactory() );
	...
	private void loadInput( IEditorInput input )
	{
		...
		if ( input instanceof IFileEditorInput )
		{
			...
			try
			{
				...
				theDiagram = (OMTPresentationDiagram)theResource.getContents().get( 0 );
			}
			catch ( IOException e )
			{
				e.printStackTrace();
