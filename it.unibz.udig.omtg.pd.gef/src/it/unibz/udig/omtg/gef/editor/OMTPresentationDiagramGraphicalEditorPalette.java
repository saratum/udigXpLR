...
import it.unibz.udig.omtg.gef.factories.OMTPresentationSpecificationFactory;
import it.unibz.udig.omtg.gef.factories.OMTTPOperationFactory;
...
public class OMTPresentationDiagramGraphicalEditorPalette extends PaletteRoot
{

	public OMTPresentationDiagramGraphicalEditorPalette()
	{
		add( addGeneralTools() );
		add( addOMTGeoreferencedClassTool() );
		add( addOMTGeoreferencedClassTool2() );
		add( addOMTPresentationSpecificationTool() );
		add( addOMTTPOperationTool() );
	}
	...
	private PaletteEntry addOMTPresentationSpecificationTool( )
	{
		PaletteDrawer group = new PaletteDrawer( "Presentation Specification" );
		CreationToolEntry entry;

		entry = new CreationToolEntry( " Presentation Specification",
				"Create a new Presentation Specification", new OMTPresentationSpecificationFactory(), null,
				null );
		entry.setToolClass( CreateAndDirectEditTool.class );
		group.add( entry );
	...
	private PaletteEntry addOMTTPOperationTool( )
	{
		PaletteDrawer group = new PaletteDrawer( "TP Operation" );
		ConnectionCreationToolEntry entry;

		entry = new ConnectionCreationToolEntry( " TP Operation", "Create a new TP Operation",
				new OMTTPOperationFactory(), null, null );
		group.add( entry );
